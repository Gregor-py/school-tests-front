...
const SelectClass: FC<SelectClass> = ({ testId, schoolClass }) => {
	const { mutate: changeClass, isLoading: isLoadingMutation } = useMutation('change test class', (data: { testId: string, schoolClass: number }) => EditTestService.changeClass(data.testId, data.schoolClass))
	...
	const defaultValue = options.find((classOption) => classOption.value === schoolClass)
