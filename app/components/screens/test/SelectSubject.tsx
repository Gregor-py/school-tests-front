...
const SelectSubject: FC<SelectSubject> = ({ testId, subjectId }) => {
	const { data, isLoading } = useQuery(
		['get subjects for edit test'],
		() => SubjectService.getAllSubjects(),
		{
			select({ data }) {
				return data.map(subject => ({ label: subject.name, value: subject._id }))
			}
		}
	)
	const { mutate: changeSubject, isLoading: isLoadingMutation } = useMutation('change test subject', (data: { testId: string, newSubject: string }) => EditTestService.changeSubject(data.testId, data.newSubject))
