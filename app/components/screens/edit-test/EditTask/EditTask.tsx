...
const EditTask: FC<{ taskId: string; testId: string }> = ({
  taskId,
  testId
}) => {
  ...
  const { mutate: changeQuestion } = useMutation(
    'change task question',
    (data: { taskId: string; newQuestion: string }) =>
      EditTaskService.changeTaskQuestion(data.taskId, data.newQuestion)
  );

  useDebouncedMutation(
    () => changeQuestion({ newQuestion: questionInput, taskId }),
    600,
    questionInput
  );
  ...
  return (
    <EditTestBlock className="mt-4">
      ...
      <div>
        {data.answerVariants.map((answer) => (
          <EditAnswerVariant
